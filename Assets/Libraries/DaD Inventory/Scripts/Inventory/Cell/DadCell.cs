...
public class DadCell : MonoBehaviour, IDropHandler
{
	...
	public Color empty = Color.white;
	...
	/// <summary>
	/// Create the DaD item from prefab and put into this cell. Old item will be destroyed.
	/// </summary>
	/// <param name="itemPrefab">Item prefab.</param>
	public void AddItemFromPrefab(GameObject itemPrefab)
	{
		if (itemPrefab != null)
		{
			RemoveItem();                                            	// Remove current item from this cell
	...
	private void SendGroupNotification(GameObject source, DadEventDescriptor desc)
	{
		if (source != null && desc != null)
		{
			...
			AccessUtility.SendMessageUpwards(source.transform, "OnDadGroupEvent", desc);
		}
	}
	...
	private void SendCellNotification(GameObject source, DadEventDescriptor desc)
	{
		if (source != null && desc != null)
		{
			...
			AccessUtility.SendMessage(source.transform, "OnDadCellEvent", desc);
		}
	}
